...
###45436
Da_45 <- as.matrix(read.csv("arraydata45436.csv",header = T))
Da_used_45 <-as.matrix(Da_45[,-1])#dim(Da_used_45)
D_ID_45 <-as.numeric(Da_45[,1])#length(D_ID_45)#22836
rownames(Da_used_45) <- D_ID_45
#rownames(Da_used_45)[1:2]
Inter <- intersect(as.numeric(rownames(Da_used_45)),
...
used_45 <- Da_used_45[which(D_ID_45 %in% Inter),]
lable_45 <- apply(as.matrix(colnames(Da_45)[-1]),1, function(x) substring(x,2,2))
lable_45_new <-lable_45
lable_45_new[which(as.numeric(lable_45)==0)] <-c("nontumor")
lable_45_new[which(as.numeric(lable_45)==1)] <-c("tumor")
lable_45_new <- as.factor(lable_45_new)
used_Module <- ID_used
used_Module_DE <- intersect(used_Module,as.numeric(DE))
gg <- which(as.numeric(rownames(used_45)) %in% used_Module_DE)
length(gg)
library(pracma)
k1 <- which(as.numeric(rownames(used_45)) %in% used_Module)
k2 <- which(as.numeric(rownames(used_T_In)) 
            %in% as.numeric(rownames(used_45))[k1])#k2
#length(k2)
...
auc_train
Data_input_45 <- as.matrix(apply(as.matrix(used_45),2,
                                 function(x)as.numeric(x)))
#a11 <-max(Data_input_45) 
#a12 <- min(Data_input_45)
Data_input_45 <- apply(Data_input_45,2,function(x)(x-z12)/(z11-z12))
dim(Data_input_45)
Data_input_45 <- t(Data_input_45[k1,])
dim(Data_input_45)
#Data_input_45 <- apply(Data_input_45,1,function(x) as.numeric(x))#dim(Data_input)
pre_new_45 <- predict(best_mod,Data_input_45,decision.values=TRUE) 
f_new_45 <- attr(pre_new_45,"decision.values")#提取决策值
prb1_new_45 <- 1/(1+exp(-f_new_45))#

show_45<-roc(as.vector(lable_45_new),as.vector(prb1_new_45),
             plot=F,print.auc=F,print.thres=F,col="blue",
             lty=1,add=F) #
auc_45 <-as.numeric(show_45[["auc"]])
auc_45
zz <- show_45[["sensitivities"]]+show_45[["specificities"]]
k <- which(zz==max(zz))
se_cur <- show_45[["sensitivities"]][k[1]]
sp_cur <- show_45[["specificities"]][k[1]]
auc_cur <- as.numeric(show_45[["auc"]])
...
Index
saveRDS(show_45,"GSE45436_0406_new.RDS")
